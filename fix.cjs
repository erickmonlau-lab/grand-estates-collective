...
content = content.replace(/Administracin/g, 'Administración')
                 .replace(/Inmobiliaria y Asesora Jurdica en el ǭrea/g, 'Inmobiliaria y Asesoría Jurídica en el área')
                 .replace(/Informacin/g, 'Información')
                 .replace(/direccin/g, 'dirección')
                 .replace(/Banǧs/g, 'Banús')
                 .replace(/telǸfono/g, 'teléfono')
                 .replace(/rǭpida/g, 'rápida')
                 .replace(/telfono/g, 'teléfono')
                 .replace(/Administracin/g, 'Administración')
                 .replace(/Asesora Jurdica/g, 'Asesoría Jurídica')
                 .replace(/rea/g, 'área')
                 .replace(/direccin/g, 'dirección')
                 .replace(/Navegacin rpida/g, 'Navegación rápida')
                 .replace(/informacin/g, 'información');
...
const contactRegex = /<section id="contacto".*?<\/section>/s;
const newContact = `        <section id="contacto" className="py-28 md:py-36 px-6 md:px-12 bg-slate-50 text-slate-900 relative">
...
// 3. Fix FormField
const formFieldRegex = /function FormField.*?<\/div>\s*\);\s*}/s;
const newFormField = `function FormField({ label, placeholder, type = "text", textarea }: { label: string; placeholder: string; type?: string; textarea?: boolean }) {
  return (
    <div>
...
content = content.replace(formFieldRegex, newFormField);

// 4. Fix Footer
const footerRegex = /<footer className="bg-onyx text-alabaster\/50">.*?<\/footer>/s;
