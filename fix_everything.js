...
content = content.replace(/Ã©/g, 'é')
                 ...
                 .replace(/o/g, 'á')
                 .replace(/?/g, 'é')
                 .replace(//g, 'ó'); // we'll just fix the obvious ones manually

// Fix specific words
content = content.replace(/Telófon/g, 'Telèfon')
                 .replace(/telófono/g, 'teléfono')
                 .replace(/Telófono/g, 'Teléfono')
                 .replace(/Solólicita/g, 'Sol·licita')
                 .replace(/tócnica/g, 'técnica')
                 .replace(/gratuóta/g, 'gratuïta')
                 .replace(/Administració de Finques/g, 'Administració de Finques')
                 .replace(/Immobiliória/g, 'Immobiliària')
                 .replace(/Asesoróa Juródica/g, 'Asesoría Jurídica')
                 .replace(/Assessoria Juródica/g, 'Assessoria Jurídica')
                 .replace(/Gestión de Patrimonios/g, 'Gestión de Patrimonios')
                 .replace(/Gestió de Patrimonis/g, 'Gestió de Patrimonis')
                 .replace(/información/g, 'información')
                 .replace(/navegación/g, 'navegación')
                 .replace(/dirección/g, 'dirección')
                 .replace(/Banós/g, 'Banús')
                 .replace(/área/g, 'área');

// 2. Add import
if (!content.includes('FooterAnimationGSAP')) {
...
const contactFormRegex = /<section id="contacto".*?<\/section>/s;
const newContactForm = \<section id="contacto" className="py-28 md:py-36 px-6 md:px-12 bg-slate-50 text-slate-900 relative">
...
content = content.replace(contactFormRegex, newContactForm);

// 4. Fix FormField component itself
...
// 5. Replace footer to be the 4 column one with GSAP animation on top
const footerOld = /<footer className="bg-onyx text-alabaster\/50">.*?<\/footer>/s;
const footerNew = \{/* ANIMATION Y FOOTER */}
...
// Replace the old footer with the new footer block
// Notice that we have to make sure we don't duplicate the animation block if it's already there
if (content.includes('{/* ANIMATION Y FOOTER */}')) {
    const fullFooterOld = /\{\/\* ANIMATION Y FOOTER \*\/}.*?<\/footer>/s;
    content = content.replace(fullFooterOld, footerNew);
} else {
    // There is no animation block yet, just replace the footer
    content = content.replace(footerOld, footerNew);
    
    // Clean up any remaining {" "} lines that might be above it
    content = content.replace(/\{\/\* "\?"\? FOOTER "\?"\? \*\/}\\s*\{\/\* PUNTO 2: Footer ampliado con columnas de navegacin y contacto \*\/}\\s*/, '');
}
