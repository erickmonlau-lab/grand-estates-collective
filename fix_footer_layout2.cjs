...
// 1. Fix FooterAnimationGSAP.tsx
let gsapContent = fs.readFileSync('src/components/FooterAnimationGSAP.tsx', 'utf8');
gsapContent = gsapContent.replace(
  `style={{ overflow: 'visible', transform: 'scale(1.25) translateY(16px)', transformOrigin: 'bottom center' }}`,
  `style={{ overflow: 'visible', transform: 'scale(1) translateY(0)', transformOrigin: 'bottom center' }}`
);
gsapContent = gsapContent.replace(
  `className={\`\${className} w-full max-w-[600px] mx-auto\`}`,
  `className={\`\${className} w-full mx-auto\`}`
);
// Make viewBox slightly tighter to remove any remaining tiny gap
gsapContent = gsapContent.replace(
  `viewBox="0 50 580 270"`,
  `viewBox="100 50 480 270"` // cropped the left side (-200 to +100) since family starts at x=230 and door is at 104, so x=100 is safe and centers it better.
);
fs.writeFileSync('src/components/FooterAnimationGSAP.tsx', gsapContent);

// 2. Rewrite the Footer in index.tsx
let current = fs.readFileSync('src/routes/index.tsx', 'utf8');

const startFooter = current.indexOf('"<footer className=\"bg-onyx text-alabaster/50\">"
const endFooter = current.indexOf('</footer>', startFooter) + 9;

const newFooter = `"<footer className=\"bg-onyx text-alabaster/50\">"
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 pt-16 pb-0 flex flex-col lg:flex-row gap-12 relative">
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 pb-8">
            ...
            <div className="lg:col-span-1">
              <img src={logoImg} alt="Gesgrama" className="h-12 w-auto object-contain opacity-100 mb-4 brightness-0 invert" />
...
current = current.substring(0, startFooter) + newFooter + current.substring(endFooter);
