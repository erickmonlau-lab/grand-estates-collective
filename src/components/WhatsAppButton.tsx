...
export function WhatsAppButton({ language = "es" }: WhatsAppButtonProps) {
  ...
  return (
    ...
    >
      ...
      {/* WhatsApp Floating Button - GPU-accelerated CSS pulse */}
      <a
        ...
        className="relative group bg-[#075E54] hover:bg-[#054c44] active:scale-95 text-white w-11 h-11 sm:w-14 sm:h-14 rounded-full flex items-center justify-center cursor-pointer transition-transform duration-200 animate-whatsapp-pulse hover:scale-110"
        style={{
          // Full inline styles so the button renders correctly BEFORE main CSS loads.
          // Without these, Tailwind classes don't apply during the CSS download window
          // and the button becomes an enormous unstyled element.
          display: 'flex',
          alignItems: 'center',
      ...
      >
        {/* Subtle ping aura ring */}
        <span className="absolute inset-0 rounded-full bg-[#075E54] animate-ping opacity-25 pointer-events-none" />
        
        {/* Exact Official WhatsApp Vector */}
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
          className="w-6 h-6 sm:w-8 sm:h-8 fill-white group-hover:rotate-12 transition-transform duration-300 relative z-10"
