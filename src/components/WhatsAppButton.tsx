...
export function WhatsAppButton({ language = "es" }: WhatsAppButtonProps) {
  ...
  return (
    <div 
      className={`fixed bottom-3 right-3 sm:bottom-6 sm:right-6 z-40 flex items-center gap-3 transition-all duration-300 ${
    ...
    >
      ...
      {/* WhatsApp Floating Button - Official High-Contrast WhatsApp Dark Green #075E54 with 2.5s gentle pulse loop */}
      <motion.a
        ...
        animate={shouldReduceMotion ? undefined : {
          scale: [1, 1.05, 1],
          boxShadow: [
            "0 8px 25px rgba(7,94,84,0.45)",
            "0 14px 34px rgba(7,94,84,0.7)",
            "0 8px 25px rgba(7,94,84,0.45)"
          ]
        }}
        ...
        whileTap={{ scale: 0.95 }}
        className="relative group bg-[#075E54] hover:bg-[#054c44] text-white w-11 h-11 sm:w-14 sm:h-14 rounded-full flex items-center justify-center cursor-pointer transition-colors duration-200"
      >
        {/* Subtle ping aura ring */}
        <span className="absolute inset-0 rounded-full bg-[#075E54] animate-ping opacity-25 pointer-events-none"></span>
