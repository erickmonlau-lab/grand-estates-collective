...
export default function HeroCarousel(_props: HeroCarouselProps) {
  return (
    <div className="w-full flex flex-col justify-between overflow-hidden" style={{ height: '100dvh' }}>
      
      {/* ── SECCIÓN HERO (Superior) ── */}
      <section
        id="hero"
        className="relative w-full h-[64dvh] sm:h-[68dvh] flex flex-col justify-between overflow-hidden rounded-b-[3.5rem] shadow-[0_20px_50px_rgba(59,130,246,0.25)] z-10 shrink-0"
        style={{ backgroundColor: '#0a0f1d' }}
      >
        {/* 1. FORZAR EL ENCUADRE DE LA FAMILIA CON <img /> ABSOLUTA */}
        <img 
          src={heroBg} 
          alt="Familia" 
          className="absolute inset-0 w-full h-full object-cover object-[75%_top] -z-20 pointer-events-none" 
        />

        {/* 2. OVERLAY OSCURO LIMITADO AL LADO IZQUIERDO */}
        <div className="absolute inset-0 bg-gradient-to-r from-[#0a0f1d] via-[#0a0f1d]/85 to-transparent w-[70%] -z-10 pointer-events-none"></div>
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center px-6 lg:px-10 xl:px-0 w-full max-w-[1200px] mx-auto py-1">
          ...
          >
            ...
            <motion.div
              ...
              className="inline-flex items-center gap-2 bg-[#1a2332]/80 backdrop-blur-md border border-white/10 rounded-full pl-1.5 pr-3 py-1 mb-2 shadow-xl"
            >
            ...
            <motion.p
              ...
              className="text-white/80 font-light leading-relaxed mb-3 max-w-[400px] sm:max-w-[480px]"
            ...
            {/* Trust features row (Horizontal/Vertical hybrid layout) */}
            <motion.div
              variants={{ hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0, transition: { duration: 1.1, ease: expo } } }}
              className="grid grid-cols-3 gap-2 mb-3 text-white w-full max-w-[550px] relative z-10"
            >
              {/* Feature 1 */}
            ...
            <motion.div
              variants={{ hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0, transition: { duration: 1.1, ease: expo } } }}
              className="flex flex-col gap-2 w-full max-w-[340px] sm:max-w-[400px] mb-1"
            >
              {/* Button 1: White */}
              <motion.a
                href="#propiedades"
      ...
      {/* ── 3. CONSTRUCCIÓN DEL RESPLANDOR AZUL CELESTE CURVO (Sección de Estadísticas) ── */}
      <div className="relative w-full flex-1 bg-gradient-to-b from-[#0e172a] via-[#2563eb]/30 to-[#f0f7ff] pt-8 pb-10 z-20 flex flex-col justify-center px-4 shrink-0">
        ...
        >
          <div
            className="bg-white rounded-3xl p-4 sm:p-6 shadow-[0_20px_50px_rgba(0,0,0,0.06)] flex flex-col items-center border border-white"
          >
            <div className="w-full grid grid-cols-4 divide-x divide-slate-100">
              {STATS.map((s, i) => (
                <div key={i} className="flex flex-col items-center justify-start gap-2 px-0.5 sm:px-4 text-center min-w-0">
                  <div className="w-7 h-7 sm:w-10 sm:h-10 rounded-full bg-[#f0f7ff] border border-blue-100 flex items-center justify-center shrink-0">
