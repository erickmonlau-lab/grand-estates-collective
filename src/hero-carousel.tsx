...
export default function HeroCarousel(_props: HeroCarouselProps) {
  return (
    <section
      id="hero"
      className="relative w-full flex flex-col justify-between overflow-x-hidden"
      style={{ minHeight: '100dvh', backgroundColor: '#0b1221' }}
    >
      {/* ── Background Image ── */}
      <motion.img
        src={heroBg}
      ...
      <div className="w-full h-[100px] md:h-[120px] shrink-0 z-10" />

      {/* ── Main Content Area ── */}
      <div className="relative z-10 flex-1 flex flex-col justify-center px-6 lg:px-10 xl:px-0 w-full max-w-[1200px] mx-auto py-8">
        <motion.div
          initial="hidden"
          animate="visible"
          variants={{
        ...
        >
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 bg-[#1a2332]/80 backdrop-blur-md border border-white/10 rounded-full pl-1.5 pr-4 py-1.5 mb-6 shadow-xl"
          >
            <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-[#0f172a] shrink-0">
              <Home className="w-3.5 h-3.5" strokeWidth={2.5} />
            </div>
            <span className="text-[9px] md:text-[10px] font-bold tracking-[0.2em] uppercase text-white/95 pt-0.5">
          ...
          <motion.p
            ...
            className="text-white/80 font-light leading-relaxed mb-10 max-w-[400px] sm:max-w-[480px]"
          ...
          {/* Trust features row (Grid layout to prevent wrapping, perfectly aligned) */}
          <motion.div
          ...
          {/* Buttons Stack (Thick pills, centered text) */}
          <motion.div
            variants={{ hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0, transition: { duration: 1.1, ease: expo } } }}
            className="flex flex-col gap-4 w-full max-w-[340px] sm:max-w-[400px] pb-12"
          >
            {/* Button 1: White */}
            <motion.a
              href="#propiedades"
              className="group relative flex items-center justify-between w-full bg-white hover:bg-slate-50 text-[#0f172a] p-1.5 pr-6 rounded-full text-decoration-none shadow-xl"
      ...
      <div className="absolute bottom-[-1px] left-0 w-full overflow-hidden leading-[0] z-[5] pointer-events-none">
        <svg
          ...
          className="w-full h-[120px] sm:h-[180px] md:h-[220px]"
        >
      ...
      >
        ...
        >
            ...
            {STATS.map((s, i) => (
              <div key={i} className="flex flex-col items-center justify-start gap-3 sm:gap-4 px-1 sm:px-4 text-center min-w-0">
                ...
                <div className="flex flex-col items-center w-full">
                  <div className="font-black text-[15px] sm:text-2xl md:text-3xl text-[#0f172a] leading-none tracking-tight mb-1 sm:mb-2 truncate w-full">
                    {s.prefix && <span>{s.prefix}</span>}
                    <Counter to={s.num} />
                    {s.suffix && <span className="text-[0.7em] ml-[1px]">{s.suffix}</span>}
                  </div>
                  <div className="font-bold text-[6.5px] sm:text-[9px] md:text-[10px] tracking-widest uppercase text-slate-500 w-full break-words leading-[1.3] px-0.5 max-h-[3em] overflow-hidden">
                    {s.label}
                  </div>
                  <div className="w-5 sm:w-8 h-[2px] bg-[#3b82f6] mt-2 sm:mt-3 rounded-full opacity-60" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="w-full mt-6 pt-5 border-t border-slate-100 flex flex-col items-center justify-center">
            <div className="flex items-center justify-center gap-2">
              <ShieldCheck className="w-4 h-4 sm:w-5 sm:h-5 text-[#64748b] shrink-0" />
              <span className="font-bold text-[8px] sm:text-[10px] tracking-[0.2em] uppercase text-[#64748b] whitespace-nowrap">
                Tu tranquilidad, nuestro compromiso
              </span>
            </div>
            {/* The elegant thin curved smile line */}
            <svg width="140" height="12" viewBox="0 0 140 12" fill="none" xmlns="http://www.w3.org/2000/svg" className="mt-1 opacity-50">
              <path d="M0 6 Q 70 12 140 6" stroke="#3b82f6" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
          </div>
        </div>
      </motion.div>
